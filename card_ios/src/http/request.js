...
//h5+app的ajax网络请求封装
let ajaxInstance = function (data) {
	let promise = new Promise((reslove, reject) => {
		console.log('data数据2', data)

		// let baseURL = process.env.NODE_ENV === 'development' ? '/api' : config.baseURL
		...
		var tips = '';
		if (data.data.transCode == 'UPLOADPHOTO') {
			tips = '证件处理中'
		}
		let mess = tips ? tips : '加载中...'
		Toast.loading({
			message: mess,
			forbidClick: true,
			duration: 0
		...
		Object.assign(data.data, dataPub, data.data)//合并对象属性
		data.data.Sign = signFn.getSign(data.data)
		...
		xhr.onreadystatechange = function () {
			...
			if (xhr.readyState == '4' && xhr.status == '200') {
				...
				if (data.retCode == "TOKENEXP" || data.retCode == "NEEDREG") {
					store.state.token = ''
					store.state.Sign = ''
					store.state.frontSeq = ''
					localStorage.setItem('token', '')
