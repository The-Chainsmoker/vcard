<template>
	<div class="opencard" id="opencard">
		...
		<div class="count" v-if="temp == 0">
				...
				<div v-if="temp == 0" class="btn" :class="canClick?'canClick':''" @click="sureSetPwd()">
					{{btnText}}
				</div>
				
				
				
				<div v-if="activation && temp == 3">
					<div class="kkBox">
					...
					<div class="btn" style="margin-top: 20px;" @click="backFn()">
						返回
					</div>
				</div>
...
<script>
	...
	export default {
		...
		async created() {
				...
				if(this.$route.query.type && this.$route.query.type == 'activation'){
					this.title = '激活卡片'
					this.btnText = '确认激活'
