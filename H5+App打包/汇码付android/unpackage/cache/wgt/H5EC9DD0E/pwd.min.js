(function(t, e) {
	...
	jQuery.fn.extend({
		showKBD: function(e) {
			...
			if (!i[l]) i[l] = [];
			u.pwdArr = i[l];
			p ? u.text(jQuery.dataToAsterisk(u.attr("data-passwordlength")) + "|") : u.text(jQuery
				.dataToAsterisk(u.attr("data-passwordlength")));
				...
				if (/^F.*$/.test(e)) {
					switch (e) {
						...
						case "FDE":
							...
							p ? u.text(jQuery.dataToAsterisk(u.attr("data-passwordlength")) +
								"|") : u.text(jQuery.dataToAsterisk(u.attr(
								"data-passwordlength")));
							break;
						case "FST":
							b.find("." + jQuery.switchType($(this).parent().parent().hide()
								.attr("class"))).show();
							break;
						case "FSP":
							b.find("." + jQuery.switchPage($(this).parent().parent().hide()
								.attr("class"))).show();
							break;
						default:
							break
					...
					u.attr("data-passwordlength", u.pwdArr.length);
					u.attr("data-ciphertext", g.encrypt(u.pwdArr.join(",")));
					p ? u.text(jQuery.dataToAsterisk(u.attr("data-passwordlength")) + "|") : u
						.text(jQuery.dataToAsterisk(u.attr("data-passwordlength")))
				}
			});
			var v;
			...
			function m() {
				...
				var i = $("#" + s);
				var r = b.height();
				$(".pwdkbdwrap").length > 0 || $("#" + o).append(
					'<div class="pwdkbdwrap" style=\'height:' + r + "px;'></div>");
				if ($(t).height() + $(t).scrollTop() - i.offset().top - i.height() <= r) {
		...
		loadKBD: function() {
			...
			if ($("div[kbdtype='Graph']").length > 0) {
				$("div[kbdtype='Graph']").text("loading…").attr("disabled", true);
				$.post("https://online.gzekt.com/vcard/pwd/PasswordServlet", {
					type: "Graph"
				}, function(t) {
					$("#PWDKBD").append(t);
			...
			if ($("div[kbdtype='Alnum']").length > 0) {
				$("div[kbdtype='Alnum']").text("loading…").attr("disabled", true);
				$.post("https://online.gzekt.com/vcard/pwd/PasswordServlet", {
					type: "Alnum"
				}, function(t) {
					$("#PWDKBD").append(t);
			...
			if ($("div[kbdtype='Number']").length > 0) {
				$("div[kbdtype='Number']").text("loading…").attr("disabled", true);
				$.post("https://online.gzekt.com/vcard/pwd/PasswordServlet", {
					type: "Number"
				},  function(t, text, xhr){
		    //function(t) {
		    ...
		    let data_set_cookie =  xhr.getResponseHeader("sessionid")
		    //let data_set_cookie = t.match(/Set-Cookie:\s([^;]+);/)[1];
		    // console.log("data_set_cookie=",data_set_cookie);
			localStorage.setItem('sessionid',data_set_cookie)
		    //createCookie(data_set_cookie.split("=")[0],data_set_cookie.split("=")[1],1); //sets cookie for 1 year
		...
		getKBD: function() {
			if (this.attr("data-passwordlength") && this.attr("data-passwordlength") != "0") return this
				.attr("data-ciphertext");
			else return null
		},
		getLenKBD: function() {
		...
		checkPWD: function(t) {
			...
			var n = false,
				s = false;
			if (i[e] && i[e].length) n = true;
			if (i[r] && i[r].length) s = true;
			if (n && s) {
	...
	jQuery.extend({
		switchPage: function(t) {
			if (/^(.*)(Lower)(.*)$/.test(t)) return RegExp.$1 + "Upper" + RegExp.$3;
			if (/^(.*)(Upper)(.*)$/.test(t)) return RegExp.$1 + "Lower" + RegExp.$3;
			if (/^(.*)(One)(.*)$/.test(t)) return RegExp.$1 + "Two" + RegExp.$3;
			if (/^(.*)(Two)(.*)$/.test(t)) return RegExp.$1 + "One" + RegExp.$3
		},
		switchType: function(t) {
			if (/^(Graph)(AlphaCaseLower)(.*)$/.test(t)) return RegExp.$1 + "DigitSymbolPageOne" +
				RegExp.$3;
			if (/^(Graph)(AlphaCaseUpper)(.*)$/.test(t)) return RegExp.$1 + "DigitSymbolPageOne" +
				RegExp.$3;
			if (/^(.*)(DigitSymbolPageOne)(.*)$/.test(t)) return RegExp.$1 + "AlphaCaseLower" + RegExp
				.$3;
			if (/^(.*)(DigitSymbolPageTwo)(.*)$/.test(t)) return RegExp.$1 + "AlphaCaseLower" + RegExp
				.$3;
			if (/^(Alnum)(AlphaCaseLower)(.*)$/.test(t)) return RegExp.$1 + "Digit" + RegExp.$3;
			if (/^(Alnum)(AlphaCaseUpper)(.*)$/.test(t)) return RegExp.$1 + "Digit" + RegExp.$3;
			if (/^(Alnum)(Digit)(.*)$/.test(t)) return RegExp.$1 + "AlphaCaseLower" + RegExp.$3
		},
...
function BigInteger(t, e, i) {
	if (t != null)
		...
		else if (e == null && "string" != typeof t) this.fromString(t, 256);
...
function bnpFromString(t, e) {
	...
	this.clamp();
	if (n) BigInteger.ZERO.subTo(this, this)
}
...
function bnpRShiftTo(t, e) {
	...
	if (r > 0) e[this.t - i - 1] |= (this.s & s) << n;
	e.t = this.t - i;
	e.clamp()
}
...
function bnpDivRemTo(t, e, i) {
	...
	i.t = p;
	i.clamp();
	if (h > 0) i.rShiftTo(h, i);
	if (o < 0) BigInteger.ZERO.subTo(i, i)
}
